  ...
  <div :class="commentClass">
    ...
    <div class="gt-comment-content">
      <div class="gt-comment-header">
        <a class="gt-comment-username" :href="userInfoUrl">{{userName}}</a>
        <span class="gt-comment-text">{{commentedText}}</span>
        <!-- date -->
        <span class="gt-comment-date">{{commentDate}}</span>
        <!-- comment like -->
        <a class="gt-comment-like" @click="likeAction" v-if="reactions">
          <SvgSpinner
            ...
            :text="reactionCount"
          />
          <SvgSpinner v-else class="gt-ico-heart" name="heart" :text="reactionCount" />
        </a>
        <!-- edit -->
        <a v-if="enableEdit" :href="comment.html_url" class="gt-comment-edit" target="_blank">
          <SvgSpinner class="gt-ico-edit" name="edit" />
        </a>
        <a v-else class="gt-comment-reply" @click="reply">
          <SvgSpinner class="gt-ico-reply" name="reply" />
        </a>
      </div>
      <div class="gt-comment-body markdown-body" v-html="comment.body_html"></div>
    </div>
  </div>
</template>
